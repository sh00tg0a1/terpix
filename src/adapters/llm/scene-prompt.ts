...
export function buildScenePrompt(opts: ScenePromptOpts = {}): string {
  ...
  return [
    ...
    `- particles: \`particle\` snow|rain|sparks|thrust|dust, \`count\`.`,
    `  \`dust\` = ambient slow-drifting motes (atmospheric depth — pond plankton,`,
    `  dust beams, suspended pollen). Use it whenever the scene wants air/water`,
    `  body without committing to weather. ~60-120 particles is plenty.`,
    ``,
    `## Natural scale — sprite \`scale\` should reflect real-world proportions`,
    `Pick \`scale\` so subjects look right NEXT TO EACH OTHER, not in isolation.`,
    `Rules of thumb when the prompt describes a natural scene:`,
    `- small fauna (fish, frog, bird, butterfly, mouse): scale ≈ 0.6 - 1.2`,
    `- mid fauna / props (cat, lantern, bowl, lotus, mushroom): scale ≈ 1.0 - 1.6`,
    `- humans / large props (person, table, chair): scale ≈ 2.0 - 2.8`,
    `- large flora / structures (tree, mountain, building, ship): scale ≈ 2.5 - 4`,
    `So in a pond scene, a koi (~1.0) should be SMALLER than a lotus blossom`,
    `(~1.4), not larger. A bowl on a table is smaller than the table.`,
    `Only override when the prompt explicitly forces a non-natural size:`,
    `- "巨大 / huge / giant / enormous" → scale ×1.8 - ×2.5 vs natural`,
    `- "迷你 / tiny / little / small" → scale ×0.4 - ×0.6 vs natural`,
    `Then the named subject visibly dominates / shrinks against its companions.`,
    ``,
    ...
    `- \`dir\`: left|right|up|down or a diagonal (top-left|top-right|bottom-left|`,
    `  bottom-right). Optional \`ease\` (default easeInOut).`,
    `- \`wobble: <degrees>\` — rotation oscillation along the path so an organic`,
    `  subject (fish, bird, butterfly, flag, jelly, snake, falling leaf) looks`,
    `  alive instead of sliding sideways like a decal. Subtle: 6-10. Cartoony:`,
    `  14-20. Omit for rigid travelers (rocket, ball, car, arrow, raindrop).`,
    `Use motion on the SUBJECT that moves; leave static backdrops without it.`,
    `Horizontal direction also auto-mirrors sprites (left → faces left); with`,
    `\`repeat\` > 1, copies alternate facing so a school/flock isn't clones.`,
    `IMPORTANT: every sprite's default facing is RIGHT (+x). flipX=true mirrors`,
    `it to face left. So a leftward-traveling subject must have flipX=true; a`,
    `rightward one keeps flipX=false. Using motion with a horizontal \`dir\``,
    `handles this for you — only set flipX manually for static sprites whose`,
    `head/front should face left.`,
    ``,
    `## One scene vs a SEQUENCE of shots`,
    `Default: one \`background\` + \`nodes\` (a single shot). For a story that`,
    `changes over time — "first … then …", multiple locations, a beat cut — use`,
    `top-level \`"shots": [ { background, durationMs, nodes }, … ]\` instead:`,
    `each entry is a shot rendered in order, with its OWN background, duration,`,
    `and nodes. \`style\` and \`camera\` stay top-level (they apply to all shots).`,
    `Use shots ONLY for genuine scene changes; a single moment stays one shot`,
    `(animate within it via \`motion\`). Shot durations should sum to the total.`,
    ``,
