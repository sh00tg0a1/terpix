...
import { clearRegistry } from '../assets/registry.js';
import { registerBuiltins } from '../assets/builtin/index.js';
...
export function loadProject(dir: string): LoadResult<LoadedProject> {
  ...
  // Project mode: reset the registry to a clean slate (builtins + local
  // discovery dirs + this project's assets/) and SKIP the global
  // ~/.cache/terpix/assets/ dir. This is what makes a project reproducible:
  // it can never silently pick up a sprite generated by some unrelated past
  // `--gen-assets` run.
  clearRegistry();
  registerBuiltins();
  const assetsDir = join(dir, 'assets');
  const extras = existsSync(assetsDir) ? [assetsDir] : [];
  loadUserAssets({ noGlobalCache: true, extraDirs: extras });
