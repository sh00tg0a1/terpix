...
export function buildSystemPrompt(opts: PromptOpts = {}): string {
  ...
  return [
    ...
    `  \`~()[]_\\/+-*=:;'"<>\`. If the prompt is non-English or asks for`,
    `  ASCII-art (bowls, hearts, faces built from \`~()_\\/\` etc.),`,
    `  TRANSLITERATE to A-Z (e.g. "一碗热面" → "HOT NOODLES") or omit text`,
    `  entirely and rely on sprites. **Never** put non-ASCII or lowercase`,
    `  into a text layer — it will be invisible at render time.`,
