...
export function buildSystemPrompt(opts: PromptOpts = {}): string {
  ...
  blocks.push(
    [
      ...
      `  \`keyframes: [{ tMs, x?, y?, scale?, rotation?, opacity?, depth? }]\`.`,
      `  Provide at least one keyframe; for motion, provide >=2. \`rotation\` is`,
      `  degrees clockwise (honored by custom shape sprites; built-ins ignore`,
      `  it). \`depth\` only matters under an iso camera (see Camera below).`,
